...
{ )");//感觉默认128挺合理的，这里我暂时不改了。
        ...
        for(int i=0; i < size; i++){
            ...
            if(move[1]!=(-1)*i)nultrans[i]=move[1];
...
{   %d,)", 
        ...
        for(i; i < dfa.accept_states.size(); i++){
            ...
            if(i != dfa.accept_states.size()-1) result += ", ";
            if(i % 10 == 0)result += "\n";
        } 
        result +="    } ;\n";
        result += fmt::sprint(R"(static yy_state_type yy_last_accepting_state;
...
{   %d,)", 
        ...
        for(i; i < nultrans.size(); i++){
            ...
            if(i != nultrans.size()-1) result += ", ";
            if(i % 10 == 0)result += "\n";
        } 
        result += "    } ;\n";
        result += fmt::sprint(R"(#define REJECT reject_used_but_not_detected
...
ECHO;
    ...
    void gen_code_1(){
        std::string result = {};
        result += fmt::sprint(R"(#include <stdio.h>
...
#define YY_DO_BEFORE_ACTION \
	...
	*yy_cp = '\0'; \
    ...
    void gen_after_yynultrans(){
        std::string result ={};
        result += fmt::sprint(R"(#ifndef YY_SKIP_YYWRAP
#ifdef __cplusplus
...
do_action:	/* This label is used only to access EOF actions. */
    ...
    void yy_gen_yylex_after_case(){
        std::string result ={};
        result += fmt::sprint(R"||(case YY_STATE_EOF(INITIAL):
	yyterminate();
