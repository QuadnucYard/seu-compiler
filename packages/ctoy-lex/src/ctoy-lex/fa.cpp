...
void DFA::to_dot(const std::filesystem::path& path) const {
    ...
    for (auto&& [i, a] : enumerate(accept_states))
        if (a != DFABuilder::NON_ACCEPT) dot.node(i, {{"xlabel", fmt::format("#{}", a)}});
    dot.node_option("shape", "circle");
    dot.edge("\"\"", start);
    for (auto&& [u, v, w] : graph.edges())
        dot.edge(u, v,
                 isprint(w) ? (w == '"' ? fmt::to_string("'\\\"'") : fmt::format("'{}'", char(w)))
                            : fmt::to_string(w));
}
#endif
...
DFA DFABuilder::hopcroft(const DFA& dfa) const {
    ...
    while (!worklist.empty()) {
        ...
        for (auto&& [u, v, w] : dfa.graph.edges())
            if (S.test(v)) images[w].set(u);
        for (sid_t c = 0; c < MAXS; c++) {
            if (images[c].none()) continue;
    ...
    for (auto&& [i, s] : enumerate(partition)) {
        for (size_t j = 0; j < s.size(); j++) {
            if (s.test(j)) mapped[j] = static_cast<vid_t>(i);
        }
    }
...
DFA DFABuilder::subset(const NFA& nfa) const {
    ...
    for (vid_t u = 0; u < n; u++) {
        for (auto&& [v, w] : nfa.graph.iter_edges(u)) {
            if (w != EPSILON) follow_epsilon[u][w] |= epsilon_closure[v];
        }
    }
    ...
    for (size_t i = 0; i < states.size(); i++) {
        ...
        for (sid_t j = 0; j < MAXS; j++) {
            ...
            for (vid_t k = 0; k < n; k++) {
                if (!states[i].test(k)) continue;
                if (follow_epsilon[k].contains(j)) next |= follow_epsilon[k][j];
            }
...
DFA DFABuilder::reachable(const DFA& dfa) const {
    ...
    for (size_t i = 0; i < dfa.accept_states.size(); i++) {
        if (sid_t u = mapped[i]; u != -1) res.accept_states[u] = dfa.accept_states[i];
    }
    res.graph.resize(cnt);
    for (auto&& [u, v, w] : dfa.graph.edges()) {
        if (mapped[u] != -1 && mapped[v] != -1) res.graph.add_edge(mapped[u], mapped[v], w);
    }
...
std::pair<NFA, DFA> DFABuilder::join_nfa() const {
    ...
    for (auto&& [i, x] : enumerate(all_dfa)) {
        nfa.accept_states.insert(nfa.accept_states.end(), x.accept_states.begin(),
                                 x.accept_states.end());
        std::replace(nfa.accept_states.begin() + nfa.size(), nfa.accept_states.end(), DUMMY_ACCEPT,
                     static_cast<vid_t>(DUMMY_ACCEPT + i));
