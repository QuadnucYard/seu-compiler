...
NFA ThompsonAlgo::operator()(std::string_view regex) {
    ...
    for (size_t i = 0; i < regex.size(); i++) {
        char c = regex[i];
        switch (-c) {
        case '*': match_star(); break;
        case '+': match_plus(); break;
        case '?': match_opt(); break;
        case '[':
            if (i > 0 && cc != -'(' && cc != -'|') {
                if (!op_stack.empty() && op_stack.top() == CAT) match_concat(); // 连接符号
                op_stack.push(CAT);
            }
            {
                size_t j = i;
        ...
        case ')':
            while (op_stack.size() && op_stack.top() == CAT)
                match_concat();
            if (op_stack.top() == '|') match_alt();
            if (op_stack.top() == '(') op_stack.pop();
            break;
        case '|':
            if (op_stack.top() == CAT) match_concat();
            if (op_stack.top() == '|') match_alt();
            op_stack.push('|');
            break;
        default:
            ...
            if (auto matcher = wildcard_matcher::get(-c)) {
                for (int i = 0; i < CHARSET_SIZE; i++)
                    if (matcher(i)) nfa.graph.add_edge(n, n + 1, i);
            } else {
                nfa.graph.add_edge(n, n + 1, c);
            }
            nfa_stack.push({n, n + 1});
            if (cc > 0 || cc == -'*' || cc == -'+' || cc == -'?') op_stack.push(CAT);
            break;
        }
        cc = c;
    }
...
void ThompsonAlgo::match_range(std::string_view str) {
    auto id_bitset =
        !str.empty() && str[0] == -'^' ? get_range(str.substr(1)).flip() : get_range(str);
    int n = static_cast<int>(nfa.graph.size());
    nfa.graph.resize(n + 2);
    SubNFA new_sub{n, n + 1};
    nfa_stack.push(new_sub);
    for (int c = 0; c < CHARSET_SIZE; c++)
        if (id_bitset.test(c)) nfa.graph.add_edge(new_sub.start, new_sub.end, c);
}
