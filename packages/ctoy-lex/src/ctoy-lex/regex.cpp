...
std::string unescape_regex(std::string_view s) {
    ...
    auto match = [&s, &bra](char c) {
        if (bra.empty())
            throw syntax_error(fmt::format(
                "Regex parsing error for \"{}\". Receive '{}', but no bracket expected.", s, c));
        if (bra.top() != c)
            throw syntax_error(fmt::format(
                "Regex parsing error for \"{}\". Receive '{}', expected '{}'.", s, c, bra.top()));
        bra.pop();
    };

    // is [ matched
    auto is_squared = [&bra]() { return !bra.empty() && bra.top() == '['; };
    ...
    for (size_t i = 0; i < s.length(); i++) {
        ...
        if (escaped) { // Escape chars
            ...
            else if (c == 's' || c == 'S' || c == 'w' || c == 'W' || c == 'd' || c == 'D') c = -c;
            res += c;
            escaped = false;
            continue;
        }
        ...
        case '|':
        case '.': break;
        default:  is_special = false; break;
        }
        res += is_special ? -c : c;
    }
    if (quoted)
    ...
    if (!bra.empty())
        throw syntax_error(
            fmt::format("Regex parsing error for \"{}\". Unenclosed bracket '{}'.", s, bra.top()));
    return res;
}
