...
GToken GLexer::scan_noop() {
    while (in) {
        scan_skip();
        if (peek == '%') return get_directive();
        if (peek == '<') return get_tag();
        if (peek == '\'') return get_char();
        if (peek == '"') return get_string();
        if (peek == '{') return get_codeblock();
        if (std::isdigit(peek)) return get_num();
