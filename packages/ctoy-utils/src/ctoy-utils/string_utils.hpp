...
inline bool is_blank_lead(std::string_view s) { return s.length() == 0 || isblank(s[0]); }

inline std::string_view trim_left(std::string_view s) {
    if (auto p = s.find_first_not_of(" \t\r\n\v\f"); p != std::string::npos) return s.substr(p);
    return "";
}
...
inline std::string_view trim(std::string_view s) { return trim_left(trim_right(s)); }

inline bool is_quoted(std::string_view s) {
    auto l = s.length();
    if (l < 2 || s[0] != '"' || s[l - 1] != '"') return false;
    if (s[l - 2] == '\\') return false;
    return true;
}
