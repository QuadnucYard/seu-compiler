...
class basic_graph {
  private:
    ...
    using vertex_type =
        std::conditional_t<has_vertex_weight, std::tuple<edge_vec, vertex_weight_type>,
                           std::tuple<edge_vec>>;
  ...
  public:
    ...
    auto iter_edges(size_type u) const { return std::ranges::subrange(at(u).begin(), at(u).end()); }
    ...
    std::pair<basic_graph, id_vec> induce_scc() const {
        struct garbow_closure {
            ...
            garbow_closure(const basic_graph& g) : g(g), low(g.size()), scc_no(g.size(), -1) {
                for (size_t i = 0; i < g.size(); i++)
                    if (!low[i]) garbow(static_cast<id_t>(i));
            }
        ...
        for (size_t i = 0; i < g.size(); i++) {
            ...
            for (auto& e : at(i)) {
                id_t v = edge_dir(e);
                if (u != c.scc_no[v]) g2.add_edge(u, edge_dir(e, c.scc_no[v]));
            }
        }
    ...
    // std::conditional_t<has_edge_weight, std::tuple<id_t, id_t, edge_weight_type>,
    // std::tuple<id_t, id_t>>
