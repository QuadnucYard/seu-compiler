...
void YaccCodeGen::gen_inc() {
    ...
    for (auto&& t : parser.analyzer.tokens)
        if (t.is_defined) fmt::print(tab_inc_file, "\t{} = {},\n", t.name, t.num);
    tab_inc_file << "};\n";
...
void YaccCodeGen::gen_yyr() {
    ...
    temp.set_string("[[yyr2]]",
                    qy::format_array(analyzer.rules |
                                     std::views::transform([](auto&& t) { return t.rhs.size(); })));
        ...
        if (!prod.action.empty()) {
            ...
            for (size_t i = 1; i <= prod.rhs.size(); i++) {
                ...
                qy::replace_all_inplace(
                    act, fmt::format("${}", i),
                    tag.empty() ? fmt::format("(yyvsp[({}) - ({})])", i, prod.rhs.size())
