...
std::optional<sid_t> is_only(auto&& slice) {
    ...
    for (auto x : slice)
        if (x != parsing_table::ERR) {
            if (only == parsing_table::ERR) only = x;
            else if (x != only) return {};
        }
    return only;
}
...
std::tuple<bool, size_t, sid_t> parsing_table_compressed::find_embed(auto&& slice, size_t l,
    ...
    if (auto r = is_only(slice)) return {true, 0, *r};

    auto check_pass = [&slice, l, base, &tab, this](
                          bool allow_overflow) -> std::optional<std::tuple<bool, size_t, sid_t>> {
        ...
        for (size_t i = 0; i <= tab.size(); i++) {
            ...
            for (auto&& [j, x] : tl::views::enumerate(slice)) {
                ...
                if (allow_overflow ? i + j < tab.size() && !is_compat(tab[i + j], x)
                    ...
                    conf++;
                    if (cur_only == ERR) cur_only = x;
                    else if (x != ERR && x != cur_only && cur_only != ERR) not_only = true;
                }
            }
            if (strong_conflict) continue;
            if (conf == 0) // 没有冲突的
        ...
        if (least_conf != l + 1) return std::tuple{false, best, only};
        return {};
    };

    // [i, i + l)
    if (auto res = check_pass(false)) return *res;
