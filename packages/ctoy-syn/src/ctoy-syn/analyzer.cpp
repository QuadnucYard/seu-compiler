...
#include "ctoy-utils/graph.hpp"
#include "ctoy-utils/graphviz.hpp"
#include "ctoy-utils/hash.hpp"
#include "ctoy-utils/myalgo.hpp"
...
SyntacticAnalyzer::item_set SyntacticAnalyzer::item_set::get_goto(sid_t symbol) const {
    item_set result;
    for (auto& item : items) {
        if (item.has_next() && item.next() == symbol) result.items.push_back(item.next_item());
    }
...
parsing_table SyntacticAnalyzer::process(const ParserOptions& options) {
    ...
    if (!options.lr1_pda_dot.empty()) to_dot(sg, options.lr1_pda_dot);
...
string SyntacticAnalyzer::to_string(const symbol_set& set) const {
    ...
    for (size_t i = 0; i < tokens.size(); i++)
        if (set.test(i)) s.append(tokens[i].name);
    return "{" + s + "}";
}
...
string SyntacticAnalyzer::to_string(const item& it) const {
    ...
    for (size_t i = 0; i < it.prod->rhs.size(); i++) {
        ...
        if (sym >= 0) s.append(tokens[sym].name);
        else s.append(nterms[-sym].name);
    }
    if (!it.has_next()) s.push_back('.');
...
void SyntacticAnalyzer::to_dot(const state_graph& sg, const fs::path& path) const {
    ...
    const auto _to_string_f = [this](const item& it) {
        string s2{};
        for (size_t i = 0; i < tokens.size(); i++)
            if (it.follow.test(i)) s2.append(qy::graphviz::label_escape(tokens[i].name));
        s2.append("\\n");
        return s2;
    };
...
SyntacticAnalyzer::state_graph SyntacticAnalyzer::get_LR1_states() const {
    ...
    for (size_t i = 0; i < states.size(); i++) {
        ...
        for (auto& p : states[i].items)
            if (p.has_next()) nexts[p.next()].items.emplace_back(p.next_item());
...
symbol_set SyntacticAnalyzer::next_follow(const item& it) const {
    ...
    for (; j < rhs.size(); j++) {
        ...
        fo |= nterms[-s].first;
        if (!nterms[-s].nullable) break;
    }
    if (j == rhs.size()) fo |= it.follow;
    return fo;
}
...
parsing_table SyntacticAnalyzer::get_LR1_table(const state_graph& LR1_states) const {
    ...
    for (size_t i = 0; i < n_states; i++) {
        ...
        for (auto&& pair : qy::ranges::pair_range(dfa.equal_range(i))) {
            auto&& e = pair.second;
            if (e.second < 0) LR1_goto[i][-e.second] = e.first;
            else LR1_action[i][e.second] = e.first;
        }
        for (auto& it : states[i].items) {
            ...
            for (size_t j = 0; j < n_tokens; j++) {
                ...
                if (LR1_action[i][j] == parsing_table::ERR) {
                    ...
                    if (prec < m_prev) LR1_action[i][j] = -it.prod->id;
                    else if (prec > m_prev) continue;
...
parsing_table SyntacticAnalyzer::get_LALR1_table(const state_graph& LR1_states,
    ...
    auto hasher = [](item_set const& is) { return is.kern_hashcode(); };
    auto keyeq = [](item_set const& is1, item_set const& is2) { return is1.kernel_equals(is2); };
    ...
    for (auto&& kern : kernel_grouped) {
        ...
        for (size_t j = 0; j < kern.size() && can_merge; j++) {
            for (auto&& [k, item] : enumerate(states[kern[j]].items)) {
                ...
                if (!item.has_next()) follows[k] |= item.follow;
            }
            // 然后检查有无冲突
            symbol_set all_fo;
            for (auto&& fo : follows) {
        ...
        for (size_t j = 0; j < kern.size(); j++) {
            for (size_t k = 0; k < len; k++) {
                ...
                if (index < -1) LR1_table.action[kern[0]][k] = index;
            }
        }
    ...
    for (size_t i = 0; i < n_states; i++) {
        for (size_t j = 0; j < n_tokens; j++)
            if (auto& x = LALR1_action[i][j]; x > 0) x = remap[x];
        for (size_t j = 0; j < n_nonterminals; j++)
            if (auto& x = LALR1_goto[i][j]; x != parsing_table::ERR) x = remap[x];
    }
